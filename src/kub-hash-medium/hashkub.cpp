...
HashKub::HashKub(QWidget *parent) : QWidget(parent), ui(new Ui::HashKub)
{
    ...
    hashVec.resize(1);
    Sectors sec;
    hashVec[0].insert(sec, QVector<QVector<int>>());
    hashVec[0][sec].resize(1);
}
...
void HashKub::rotates(int n, Sectors &sectors, const QVector<int> &vec)
{
    if (vec.isEmpty()) {
        ...
        rotate(E, n, sectors, vec);
        rotate(E2, n, sectors, vec);
        rotate(E3, n, sectors, vec);
        ...
        rotate(M, n, sectors, vec);
        rotate(M2, n, sectors, vec);
        rotate(M3, n, sectors, vec);
        ...
        rotate(S, n, sectors, vec);
        rotate(S2, n, sectors, vec);
        rotate(S3, n, sectors, vec);
    } else {
        switch (vec.first()) {
            case U:
            case U2:
...
void HashKub::findSolution()
{
    for (int i = 0; i <= ui->spinBox->value(); ++i) {
        if (hashVec[i].contains(mSectors)) {
            for (const auto &vec : hashVec[i][mSectors]) {
                QString res;
