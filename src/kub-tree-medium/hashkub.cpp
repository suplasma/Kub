...
void HashKub::rotates(const Sectors &sectors, const QVector<uint8_t> &vec)
{
    if (vec.isEmpty()) {
        ...
        rotate(E, sectors, vec);
        rotate(E2, sectors, vec);
        rotate(E3, sectors, vec);
        ...
        rotate(M, sectors, vec);
        rotate(M2, sectors, vec);
        rotate(M3, sectors, vec);
        ...
        rotate(S, sectors, vec);
        rotate(S2, sectors, vec);
        rotate(S3, sectors, vec);
    } else {
        switch (vec.first()) {
            case U:
            case U2:
...
void HashKub::findSolution()
{
    ...
    for (int arrIndex = 1; arrIndex <= ui->spinBox->value(); ++arrIndex) {
        for (const auto &key : tree.keys(arrIndex)) {
            ...
            if (tree.size(sec) == ui->spinBox->value()) {
                flag = true;
                for (const auto &vecHash : tree.value(sec)) {
                    for (const auto &vec : tree.value(key)) {
                        switch (vec.last()) {
                            case E:
                            case E2:
